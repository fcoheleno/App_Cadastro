...
namespace App_Cadastro
{
    ...
    public partial class FormCadastro : Form
    {
        ...
        public FormCadastro(Form origem)
        {
            InitializeComponent();
            formAnterior = origem;
        }

        public FormCadastro(Form origem, string codigo, string nome, string marca, decimal valor, int quantidade)
        {
            InitializeComponent();
            formAnterior = origem;
            modoEdicao = true;
            ...
            BtnCadastro.Text = "Salvar alterações";
        }
        ...
        private void BtnCadastro_Click(object sender, EventArgs e)
        {
            ...
            string quantidadeTexto = txtQuantidade.Text.Trim().ToUpper();
            ...
            if (modoEdicao)
            {
                AtualizarProduto(codigo, nome, marca, valor, quantidade);
            }
            else
            {
                ...
                using (FbConnection conn = new FbConnection(Conexao.ConnectionString))
                {
                    try
                    {
                        ...
                        string insertQuery = "INSERT INTO PRODUTOS(CODIGO, NOME, MARCA, VALOR, QUANTIDADE, USUARIO_ID) VALUES (@codigo, @nome, @marca, @valor, @quantidade, @usuario_id)";
                        using (FbCommand insertCmd = new FbCommand(insertQuery, conn))
                        {
                            ...
                            insertCmd.Parameters.AddWithValue("@usuario_id", UsuarioLogado.Id);
        ...
        private void AtualizarProduto(string codigo, string nome, string marca, decimal valor, int quantidade)
        {
            using (FbConnection conn = Conexao.AbrirConexao()){
                try
                {
                    string updateQuery = "UPDATE PRODUTOS SET NOME = @nome, MARCA = @marca, VALOR = @valor, QUANTIDADE = @quantidade WHERE CODIGO = @codigo AND USUARIO_ID = @usuarioId";
                    using (FbCommand cmd = new FbCommand(updateQuery, conn))
                    {
